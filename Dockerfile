...
# and `pip install fraud` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "fraud", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n fraud -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "fraud", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "fraud", "YOUR_CONSOLE_SCRIPT"]
