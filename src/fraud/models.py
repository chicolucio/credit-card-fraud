...
def build_classification_model_pipeline(
    ...
    resampling: imblearn.base.BaseSampler = None,
    ...
    resampling : ImbPipeline, optional
        Resampling method, by default None.
    ...
    if resampling is None:
        ...
        if preprocessor is not None:
            ...
            pipeline = ImbPipeline([("resampling", resampling), ("clf", classifier)])
...
def grid_search_cv_classifier(
    ...
    GridSearchCV
        Grid search object.
    """
    model = build_classification_model_pipeline(classifier, preprocessor, resampling)

    # check if model has predict_proba and remove metrics that require it
    metrics = MODEL_EVALUATION_METRICS.copy()
    check_predict_proba = getattr(model, "predict_proba", None)
    if check_predict_proba is None:
        metrics.remove("neg_brier_score")
